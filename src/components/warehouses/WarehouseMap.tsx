...
export default function WarehouseMap({ warehouses }: Props) {
  const validWarehouses = warehouses.filter(
    w => w.address?.latitude && w.address?.longitude
  );

  if (validWarehouses.length === 0) return null;

  const center: [number, number] = [
    validWarehouses[0].address.latitude,
    validWarehouses[0].address.longitude,
  ];
  ...
  return (
    ...
    >
      ...
      {validWarehouses.map(wh => (
        <Marker
          key={wh.id}
          position={[wh.address.latitude, wh.address.longitude]}
        >
          <Popup>
