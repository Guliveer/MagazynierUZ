...
export default function ProductsPage() {
  ...
  const updateURL = useCallback(
    (newFilters: ProductFilterValues, newPage: number, newPageSize: number, newSortBy: SortField, newSortDir: SortDirection, newViewMode: ViewMode) => {
      ...
      if (newFilters.warehouseId) params.set("warehouse", newFilters.warehouseId.toString());
      if (newFilters.locationId) params.set("location", newFilters.locationId.toString());
      if (newFilters.minPrice) params.set("minPrice", newFilters.minPrice);
      if (newFilters.maxPrice) params.set("maxPrice", newFilters.maxPrice);
      if (newFilters.minQuantity) params.set("minQty", newFilters.minQuantity);
      if (newFilters.maxQuantity) params.set("maxQty", newFilters.maxQuantity);
      if (newFilters.isAvailable) params.set("available", "true");
      if (newPage > 1) params.set("page", newPage.toString());
      if (newPageSize !== 25) params.set("size", newPageSize.toString());
      if (newSortBy !== "name") params.set("sortBy", newSortBy);
      if (newSortDir !== "asc") params.set("sortDir", newSortDir);
      if (newViewMode !== "table") params.set("view", newViewMode);
  ...
  const performSearch = useCallback(async () => {
    try {
      ...
      if (filters.minQuantity) params.minQuantity = parseInt(filters.minQuantity, 10);
      if (filters.maxQuantity) params.maxQuantity = parseInt(filters.maxQuantity, 10);
      if (filters.isAvailable) params.isAvailable = true;
  ...
  // Sort products locally (since API doesn't support sorting)
  ...
  const handleDeleteConfirm = async () => {
    if (!selectedProduct || !filters.warehouseId || !filters.locationId) return;
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      ...
      {/* Search results */}
      {hasSearched && <ProductSearchResults products={paginatedProducts} isLoading={isSearching} searchTerm={filters.searchQuery} sortBy={sortBy} sortDirection={sortDirection} viewMode={viewMode} page={page} pageSize={pageSize} totalResults={sortedProducts.length} onSort={handleSort} onViewModeChange={handleViewModeChange} onPageChange={handlePageChange} onPageSizeChange={handlePageSizeChange} onEdit={handleEditClick} onDelete={handleDeleteClick} />}
